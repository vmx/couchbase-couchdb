    ...
    false ->
        ...
        true ->
            ...
            _ ->
                ...
                % (start seq > 0) also the first snapshot needs to contain
            ...
            create_mutations_dups(Mutations3, ItemsPerSnapshot,
                DupsPerSnapshot);
        false ->
...
-spec create_mutations_dups([tuple()], pos_integer(), pos_integer()) ->
                                         [tuple()].
create_mutations_dups(Mutations, ItemsPerSnapshot,
        DupsPerSnapshot) ->
    % Make sure every test run leads to the same result
    random:seed(5, 6, 7),
    Mutations2 = lists:foldl(fun(Mutation, Acc) ->
        NumAcc = length(Acc),
        case NumAcc > 0 andalso (NumAcc rem ItemsPerSnapshot) =:= 0 of
        true ->
            RandomMutations = lists:map(fun(_) ->
                Nth = random:uniform(NumAcc),
                lists:nth(Nth, Acc)
            end, lists:seq(1, DupsPerSnapshot)),
            Acc ++ RandomMutations ++ [Mutation];
        false ->
            Acc ++ [Mutation]
        end
    end, [], Mutations),
    StartSeq = element(2, hd(Mutations)),
    % Apply sequention sequence numbers
    {Mutations3, _} = lists:mapfoldl(fun(M, I) ->
        {setelement(2, M, I), I + 1}
    end, StartSeq, Mutations2),
    Mutations3.
